# BNF-K 데이터 파이프라인 가이드 (350 종목 유니버스)

이 문서는 유니버스(KOSPI200 + KOSDAQ150) 350종목에 대해 데이터 리필 및 증분 업데이트를 수행하는 절차를 설명합니다.

## 1. 개요
본 시스템은 오직 고정된 350개 종목만을 대상으로 하며, 모든 선별/매매/분석은 이 유니버스 안에서만 이루어집니다.

- **유니버스**: `data/universe_kospi200.csv` + `data/universe_kosdaq150.csv` (총 350개)
...
### Step 1: 유니버스 초기화
유니버스 멤버를 DB에 등록하고 `stock_info`를 350개로 고정합니다.
```bash
python -m src.collectors.universe_loader
```
* 결과: `universe_members` 테이블에 350개 적재, `stock_info` 350개로 초기화.
...
1. **백엔드 실행**: `python server.py` (5001 포트)
2. **프론트엔드 실행**: `cd frontend && npm run dev` (5173 포트)
3. **확인 항목**:
   - **데이터 준비도**: 유니버스 350개 중 몇 개가 수집되었는지, 결측치는 없는지 표시.
...
- 유니버스 350개 외의 데이터는 수집되지 않으며, 선별 엔진에서도 제외됩니다.
