...
코스피200 + 코스닥150(≈350종목)을 **실시간(또는 준실시간) 감시**하면서
**API 호출 제약을 피하고**, 조건 충족 시 **텔레그램/디스코드로 신호를 보내는 기능**을 구현하기 위한 “운영 설계 + 구현 스펙”입니다.

> 핵심 아이디어(하이브리드)
>
> 1) **REST 배치 스캔**(멀티종목 시세조회)으로 350개 전체를 **30~60초 간격**으로 훑고  
...
### 목표
- 350종목을 상시 감시하면서도 **REST 호출량 폭발을 방지**
- “조건 근접 종목”만 실시간 정밀 감시(동적 구독)
- 신호 발생 시 알림(텔레그램/디스코드/로그)
- 재연결/재구독/중복 알림 방지 등 **운영 안정성**

### 비목표
- “모든 350종목을 tick 단위로 상시 실시간 스트리밍” (트래픽/제한/운영 난이도 과다)
...
### 2.2 데이터 흐름(요약)
1) `scanner`가 30~60초마다 `intstock_multprice`로 350종목 스냅샷 수집
...
### 3.1 용어
- **스캔(Scan)**: REST 배치로 전체 350종목을 저빈도로 훑는 것(예: 60초)
...
- 멀티종목 호출은 “배치 크기(batch_size)”에 따라 호출 횟수 결정:
  - `calls_per_scan = ceil(350 / batch_size)`
- 초기 권장:
  - `batch_size = 50`
  - `scan_interval = 60s`
...
> 350종목 “상시 tick 스트리밍”은 비추천. 먼저 A로 안정화 후 B로 확장.
...
- (금지) 350종목을 단일종목 REST API로 초단위 폴링
